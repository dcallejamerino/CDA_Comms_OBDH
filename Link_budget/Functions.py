...
import warnings
#from LinkBudget import LinkBudget, distance, path_loss, received_power, noise_power, expected_snr, visibility_time
...
class LinkBudget:
    ...
    def expected_snr(self, f0, Pt, Gt, Gr, B, Tnoise, eta_t, eta_r, Lt, Lr, Ladd, gamma):
        #Inputs:
            # f0 - carrier frequency (Hz)
            # Pt - transmitted power (Watts)
            # Gt - transmitt antenna gain (dBi)
        ...
        Pr = self.received_power(Pt, f0, Gt, Gr, eta_t, eta_r, Lt, Lr, Ladd) #Receiver power
        N = self.noise_power(B, Tnoise, gamma) #Noise power
        snr = Pr - N
        return snr, EIRP
